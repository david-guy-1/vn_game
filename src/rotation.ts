function rotate(p : point, origin : point, amt : number) : point {  // clockwise rotation programming convention, ccw math convention
    var dx = p[0] - origin[0];
    var dy = p[1] - origin[1];
...
function displace_fillstyle(style : fillstyle, amt : point ) : fillstyle {
...
function rotate_fillstyle(style : fillstyle,origin : point, amt : number) : fillstyle {
    var x = JSON.parse(JSON.stringify(style)) as fillstyle;
    if(typeof(x) == "string"){
        return x;
    }
...
function scale_fillstyle(style : fillstyle,center : point, x_amt : number, y_amt : number) : fillstyle {
    var x = JSON.parse(JSON.stringify(style)) as fillstyle;
    if(typeof(x) == "string"){
        return x;
    }
...
function displace_command(command : draw_command, amt : point) : draw_command {
...
function scale_number(number : number, center : number, factor:number ): number{
    return (number - center) * factor + center; 
}

function scale_command(command : draw_command, center : point, x_amt : number, y_amt : number) : draw_command {
    var new_command : draw_command = JSON.parse(JSON.stringify(command)); // copy it
    switch(new_command.type){
        case "drawCircle":
        case "drawPolygon":
...
function rotate_command(command : draw_command, origin : point, amt : number) : draw_command{
    var new_command : draw_command = JSON.parse(JSON.stringify(command)); // copy it
    switch(new_command.type){
        case "drawCircle":
        case "drawPolygon":
